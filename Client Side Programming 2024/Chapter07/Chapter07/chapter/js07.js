 ...
 */
      ...
      document.getElementById("getFile").onchange = function() {
            ...
            let userFile = this.files[0]; //File object selected by the user
            //Files collection contains the files selected by the user
            ...
            try 
            {
                  ...
                  let fr = new FileReader(); //Create a FileReader object
                  fr.readAsText(userFile); //Load userFile as a text file
                  ...
                  fr.onload = function() {
                        ...
                        let sourceText = sourceDoc.textContent; //Store the text of the source document without the HTML tags

                        //Generate the word cloud
                        wordCloud(sourceText); //Call the wordCloud() function using sourceText as the argument

                  } //When the document is loaded, write its contents to the web page
            ...
            function wordCloud(sourceText) { //The wordCloud function()
                  //Convert the source text to lowercase
                  //and remove leading and trailing whitespace
                  sourceText = sourceText.toLowerCase(); //Convert characters in sourceText to lowercase
                  sourceText = sourceText.trim(); //Remove leading and trailing whitespace

                  //Leave only alphabet characters and whitespace in the text
                  let alphaRegx = /[^a-zA-Z\s]/g; //Select characters that are not alphabetic nor whitespace
                  sourceText = sourceText.replace(alphaRegx, ""); //Replace those characters with an empty text string
                 
                  //Remove stop words from the text
                  for (let i = 0; i < stopWords.length; i++) {
                        let stopRegx = new RegExp ("\\b" + stopWords[i] + "\\b", "g"); //Select all stop words in the document
                        sourceText = sourceText.replace(stopRegx, ""); //Replace stop words with empty text strings
                  }

                  //Place the remaining words in array
                  let words = sourceText.split(/\s+/g);
                  
                  //Split the document at locations of one or more whitespace characters
